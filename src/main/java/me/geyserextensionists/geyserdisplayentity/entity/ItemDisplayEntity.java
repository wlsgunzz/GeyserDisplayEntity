...
public class ItemDisplayEntity extends SlotDisplayEntity {
    ...
    // tracks spawned furniture for seat-offset per-item lookup
    ...
    // exposes the entity's own facing direction for rotation-aware calculations done outside
    // this class (yaw itself is protected on the base Entity type)
    public float getYaw() {
        return this.yaw;
    }
    ...
    public Vector3f bedrockPosition() {
        ...
        if (xOff == 0.0 && zOff == 0.0) {
            return super.bedrockPosition().add(Vector3f.from(0, yOff, 0));
        }
